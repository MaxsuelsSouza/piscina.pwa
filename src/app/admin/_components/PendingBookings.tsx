    ...
    <div className="lg:col-span-1">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 h-full">
        <h2 className="text-lg font-light text-gray-900 dark:text-gray-100 mb-4 pb-4 border-b border-gray-100 dark:border-gray-700">
          Pendentes <span className="text-sm text-gray-400 dark:text-gray-400 font-light">({pendingBookings.length})</span>
        </h2>
        {pendingBookings.length > 0 ? (
          <div className="space-y-3 overflow-y-auto max-h-[600px] pr-2">
            {pendingBookings.map(booking => {
              ...
              return (
                <div key={booking.id} className="p-4 bg-gray-50 dark:bg-gray-700 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-all">
                  <div className="mb-3">
                    <h3 className="font-medium text-gray-900 dark:text-gray-100 text-sm mb-2">
                      {booking.customerName}
                    </h3>
                    {expiresIn > 0 && (
                      <span className="inline-block px-2.5 py-1 text-xs rounded-full bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 font-medium">
                        ⏱ {expiresIn}min
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="flex-1 px-3 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-all text-xs font-medium"
                    >
                      Ver mais
                    </button>
                  </div>
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <div className="w-16 h-16 bg-gray-50 dark:bg-gray-700 rounded-2xl flex items-center justify-center mb-4">
              <svg className="w-8 h-8 text-gray-300 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <p className="text-sm text-gray-400 dark:text-gray-300 font-light">Nenhum agendamento pendente</p>
          </div>
        )}
      </div>
    </div>
